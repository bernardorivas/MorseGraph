#!/usr/bin/env python3
"""
MorseGraph Example 3: Adaptive Refinement

This script shows how to compute a Morse Graph with iterative refinement.

The iterative_morse_computation function progressively subdivides
specific Morse sets using AdaptiveGrid to focus resolution on complex regions.

As an example, we analyze the Van der Pol oscillator with targeted refinement,
demonstrating how to adaptively increase detail where needed.
"""
...
def main():
    print("MorseGraph Example 3: Iterative Refinement")
    print("=========================================")
    ...
    # Create an adaptive grid starting with a single box
    grid = AdaptiveGrid(bounds=domain, max_depth=8)

    # Perform initial uniform subdivision (similar to CMGDB's subdiv_init)
    # This is necessary because the adaptive refinement requires >1 box
    # to detect regions of dynamical complexity
    initial_subdivisions = 5
    for _ in range(initial_subdivisions):
        grid.subdivide()  # Subdivide all current leaves

    # Create the model
    model = Model(grid, dynamics)

    print(f"Initial grid: {len(grid.get_boxes())} boxes (after {initial_subdivisions} uniform subdivisions)")
    ...
    for i, iteration_info in enumerate(refinement_history):
        print(f"Iteration {iteration_info['iteration']}: "
              ...
              f"{iteration_info['num_boxes_to_refine']} boxes to refine")
    ...
    progress_plot_path = os.path.join(figures_dir, "3_refinement_progress.png")
    ...
    final_plot_path = os.path.join(figures_dir, "3_adaptive_refinement_final.png")
    ...
    # Create initial computation for comparison (same initial subdivision as before)
    initial_grid = AdaptiveGrid(bounds=domain, max_depth=8)
    for _ in range(initial_subdivisions):
        initial_grid.subdivide()
    ...
    # Plot initial (coarse uniform grid)
    plot_morse_sets(initial_grid, initial_morse_graph, ax=ax1)
    ax1.set_title(f"Initial Coarse Grid\n({len(initial_grid.get_boxes())} boxes)")
    ax1.set_xlabel("x")
    ax1.set_ylabel("y")
    ...
    comparison_plot_path = os.path.join(figures_dir, "3_refinement_comparison.png")
