"""
Plotting and Visualization Utilities for Leslie Map 3D
...
def plot_morse_graph_to_ax(morse_graph, ax, title='', cmap=plt.cm.cool):
    ...
    # Render as PNG (DPI is controlled by the final figure save, not here)
...
def plot_morse_graph_comparison(
    ...
    - Top row: Ground truth MG, Latent full MG, Latent train MG, Latent val MG
    - Bottom row: 3D barycenters, Full 2D overlay, Train 2D overlay, Val 2D overlay

    Args:
        ground_truth_morse_graph: Precomputed CMGDB morse graph for Leslie map
        ground_truth_barycenters: Numpy archive with barycenter data
    ...
    z_barycenters = None
    barycenters_3d_list = []
    if ground_truth_barycenters is not None:
        # Handle both old format ('barycenters' key) and new format ('morse_set_N' keys)
        if 'barycenters' in ground_truth_barycenters:
            barycenters_3d_list = ground_truth_barycenters['barycenters']
        else:
            # New format: morse_set_0, morse_set_1, etc.
            morse_set_keys = sorted([k for k in ground_truth_barycenters.keys() if k.startswith('morse_set_')],
                                   key=lambda x: int(x.split('_')[-1]))
            barycenters_3d_list = [ground_truth_barycenters[k] for k in morse_set_keys]
    ...
    # Get colors for ground truth barycenters using cool colormap (to match ground truth)
    ...
    for variant, config in variant_configs.items():
        ...
        # Ground truth (uses cool colormap from pre-rendered image)
        ax1 = fig.add_subplot(2, 4, 1)
        try:
            img = plt.imread(ground_truth_graph_img_path)
            ax1.imshow(img)
        ...
        if latent_mg_train:
            plot_morse_graph_to_ax(latent_mg_train, ax3, title='BoxMapData on E(X_train) - MG', cmap=latent_cmap)
        else:
            ax3.text(0.5, 0.5, 'No Morse graph', ha='center', va='center', transform=ax3.transAxes)
            ax3.set_title('BoxMapData on E(X_train) - MG')
            ax3.axis('off')

        # Latent val (use viridis)
        ax4 = fig.add_subplot(2, 4, 4)
        if latent_mg_val:
            plot_morse_graph_to_ax(latent_mg_val, ax4, title='BoxMapData on E(X_val) - MG', cmap=latent_cmap)
        else:
            ax4.text(0.5, 0.5, 'No Morse graph', ha='center', va='center', transform=ax4.transAxes)
            ax4.set_title('BoxMapData on E(X_val) - MG')
            ax4.axis('off')

        # Bottom row: 3D barycenters + 2D overlays
        # 3D barycenters (use cool colormap to match ground truth)
        ax5 = fig.add_subplot(2, 4, 5, projection='3d')
        if barycenters_3d_list:
            # Use cool colormap for 3D ground truth (matches pre-rendered Morse graph)
            ...
            for i, bary_set in enumerate(barycenters_3d_list):
                if len(bary_set) > 0:
                    ...
                    ax5.scatter(bary_set[:, 0], bary_set[:, 1], bary_set[:, 2], c=[color], s=1, marker='s', alpha=0.6)
            ax5.set_title('Scatterplot of barycenters', fontsize=10)
            ax5.set_xlabel('X'); ax5.set_ylabel('Y'); ax5.set_zlabel('Z')
        else:
            ax5.text(0.5, 0.5, 0.5, 'No barycenters', ha='center', va='center')
        ...
        _plot_2d_latent_overlay(ax6, z_train_sample, labels_full, colors_full, z_barycenters, barycenter_colors,
        ...
        _plot_2d_latent_overlay(ax7, z_train_sample, labels_train, colors_full, z_barycenters, barycenter_colors,
                                latent_mg_train, latent_bounds, config, 'BoxMapData (Training)',
                                cmap=latent_cmap, show_grey_bg=config['grey_bg'])

        # Val: 2D overlay (use viridis)
        ax8 = fig.add_subplot(2, 4, 8)
        _plot_2d_latent_overlay(ax8, z_val_sample, labels_val, colors_full, z_barycenters, barycenter_colors,
                                latent_mg_val, latent_bounds, config, 'BoxMapData (Validation)',
                                cmap=latent_cmap, show_grey_bg=config['grey_bg'])
...
def _plot_2d_latent_overlay(ax, z_data, labels, colors, z_barycenters, barycenter_colors, morse_graph, latent_bounds, config, title, cmap=plt.cm.viridis, show_grey_bg=True, show_data_boxes=True):
    ...
    # E(barycenters) as colored squares - use ground truth colors (cool colormap) - zorder=6 to be on top of everything
...
def plot_preimage_classification(
        ...
        num_samples=10000):
    """
    Generate Figure 3: Preimage Classification (3x3 grid).

    Rows:
    - Top: 3D preimages E^-1(Morse Sets) for Full/Train/Val
    - Middle: Morse Graphs for Full/Train/Val
    - Bottom: 2D Latent Morse Sets + Samples for Full/Train/Val
    ...
    if latent_mg_full:
        plot_morse_graph_to_ax(latent_mg_full, ax4, title='Full: Morse Graph')
    else:
        ax4.text(0.5, 0.5, 'No Morse graph', ha='center', va='center', transform=ax4.transAxes)
        ax4.set_title('Full: Morse Graph')
        ax4.axis('off')
    ...
    if labels_full is not None:
        ...
        ax7.scatter(z_train[:, 0], z_train[:, 1], c='lightgrey', s=1, alpha=0.3)
        # Plot classified samples with colors
        for i, color in enumerate(colors):
            mask = labels_full == i
            if np.any(mask):
                ax7.scatter(z_train[mask, 0], z_train[mask, 1], c=[color], s=5, alpha=0.8)
        ax7.set_title('Full: Latent Morse Sets + Samples', fontsize=10)
    else:
        ax7.text(0.5, 0.5, 'No Morse sets', ha='center', va='center', transform=ax7.transAxes)
        ax7.set_title('Full: Latent Morse Sets + Samples')
    ax7.set_xlabel('Latent 0'); ax7.set_ylabel('Latent 1')
    ...
    if latent_mg_train:
        plot_morse_graph_to_ax(latent_mg_train, ax5, title='Train: Morse Graph')
    else:
        ax5.text(0.5, 0.5, 'No Morse graph', ha='center', va='center', transform=ax5.transAxes)
        ax5.set_title('Train: Morse Graph')
        ax5.axis('off')

    # Bottom: Latent morse sets + samples
    ax8 = fig.add_subplot(3, 3, 8)
    if labels_train is not None and np.any(labels_train >= 0):
        ax8.scatter(z_train[:, 0], z_train[:, 1], c='lightgrey', s=1, alpha=0.3)
        for i, color in enumerate(colors):
            mask = labels_train == i
            if np.any(mask):
                ax8.scatter(z_train[mask, 0], z_train[mask, 1], c=[color], s=5, alpha=0.8)
        ax8.set_title('Train: Latent Morse Sets + Samples', fontsize=10)
    else:
        ax8.text(0.5, 0.5, 'No Morse sets', ha='center', va='center', transform=ax8.transAxes)
        ax8.set_title('Train: Latent Morse Sets + Samples')
    ax8.set_xlabel('Latent 0'); ax8.set_ylabel('Latent 1')
    ...
    if latent_mg_val:
        plot_morse_graph_to_ax(latent_mg_val, ax6, title='Val: Morse Graph')
    else:
        ax6.text(0.5, 0.5, 'No Morse graph', ha='center', va='center', transform=ax6.transAxes)
        ax6.set_title('Val: Morse Graph')
        ax6.axis('off')

    # Bottom: Latent morse sets + samples
    ax9 = fig.add_subplot(3, 3, 9)
    if labels_val is not None and np.any(labels_val >= 0):
        ax9.scatter(z_val[:, 0], z_val[:, 1], c='lightgrey', s=1, alpha=0.3)
        for i, color in enumerate(colors):
            mask = labels_val == i
            if np.any(mask):
                ax9.scatter(z_val[mask, 0], z_val[mask, 1], c=[color], s=5, alpha=0.8)
        ax9.set_title('Val: Latent Morse Sets + Samples', fontsize=10)
    else:
        ax9.text(0.5, 0.5, 'No Morse sets', ha='center', va='center', transform=ax9.transAxes)
        ax9.set_title('Val: Latent Morse Sets + Samples')
