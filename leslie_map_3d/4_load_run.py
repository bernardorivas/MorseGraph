...
from leslie_modules import Config
from leslie_modules.plotting import (
...
from leslie_modules.learning import (
    compute_latent_bounds,
    compute_latent_morse_graph_cmgdb,
    compute_latent_morse_graph_data_restricted_cmgdb
)
from leslie_modules.ground_truth import load_ground_truth_barycenters
...
def load_run_and_regenerate_plots(run_path, force=False, skip_morse=False, verbose=True):
    ...
    if skip_morse:
        ...
        latent_morse_graph_train, _ = compute_latent_morse_graph_data_restricted_cmgdb(
            latent_dynamics, device, z_train, latent_bounds.tolist(), config
        )
        if verbose:
            print(f"  ✓ Train-restricted MG: {latent_morse_graph_train.num_vertices()} Morse sets")

        latent_morse_graph_val, _ = compute_latent_morse_graph_data_restricted_cmgdb(
            latent_dynamics, device, z_val, latent_bounds.tolist(), config
        )
        if verbose:
    ...
    if not skip_morse:
        morse_graph_paths = [
            ...
            ("morse_graph_latent_train.png", latent_morse_graph_train, "BoxMapData on E(X_train) - Morse Graph"),
            ("morse_graph_latent_val.png", latent_morse_graph_val, "BoxMapData on E(X_val) - Morse Graph")
        ]
